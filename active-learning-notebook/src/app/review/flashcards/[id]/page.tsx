...
export default function NoteFlashcardsPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (noteId) fetchSharedCards(noteId);
  }, [noteId]);

  const fetchSharedCards = async (id: string) => {
    const { data: { user } } = await supabase.auth.getUser();
    setIsGuest(!user);

    const { data: note, error } = await supabase.from("notes").select("title, content, course_topic").eq("id", id).single();
    
    if (error || !note || note.course_topic !== "SHARED_FLASHCARDS") {
       setLoading(false);
       return;
    }
    ...
    try {
      const parsedCards = JSON.parse(note.content);
