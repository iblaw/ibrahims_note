...
export default function EditScheduleModal({ schedule, courses, onClose, onUpdated }: { schedule: any, courses: any[], onClose: () => void, onUpdated: () => void }) {
  const [name, setName] = useState(schedule.name);
  const [targetDate, setTargetDate] = useState(new Date(schedule.target_date).toISOString().split('T')[0]);
  const [weeklyHours, setWeeklyHours] = useState<number>(schedule.weekly_hours);
  const [selectedCourses, setSelectedCourses] = useState<string[]>(schedule.course_ids || []);
  ...
  return (
    <div className="fixed inset-0 bg-neutral-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[2rem] p-6 md:p-8 w-full max-w-lg shadow-2xl relative border-2 border-orange-200">
