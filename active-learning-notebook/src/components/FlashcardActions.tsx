...
export default function FlashcardActions({ flashcards, defaultTitle = "Grand Flashcards", onSaved }: { flashcards: any[], defaultTitle?: string, onSaved?: (deck: any) => void }) {
  ...
  const confirmAction = async () => {
    ...
    try {
      ...
      console.error(`Failed to ${modalAction} deck`, err);
      alert(`Failed to ${modalAction} deck.`);
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3">
              ...
              >
                {modalAction === "share" ? "Share & Copy Link" : "Save Deck"}
              </button>
            </div>
          </div>
        </div>
